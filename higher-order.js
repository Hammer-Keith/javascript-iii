...
let totalPopulation = populations.reduce((accume, curr) => accume + curr)
...
var myStrongest = monstersInYourPocket.filter(x => x.CP>200)
...
let ordersTotal = orders.map(x=>x.price * x.tax + x.price)
...
let prebobsTotal = purchases.filter(x=>x.owner === 'Bob')
console.log(prebobsTotal)
var x = 0;
for (var i = 0;i<prebobsTotal.length;i++){
  x += prebobsTotal[i].price
}
let bobsTotal = x;